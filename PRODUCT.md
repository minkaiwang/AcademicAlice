...
| 项 | 当前值 | 代码位置 |
|----|--------|----------|
| 产品显示名 | **学术爱丽丝** | `app_brand.py` → `APP_DISPLAY_NAME` |
...
**学术爱丽丝**是在 Windows 上常驻运行的 **桌面陪伴 + 学术场景助手**：在保留桌宠形态与音乐、语音、AI 聊天等能力的同时，通过 **爱丽丝科研工作台**（浏览器内本地页）管理 **论文、项目与任务进度**（具体能力以 `resc/workbench` 与 `PROGRESS.md` 为准）。
...
**短版（一两句）**  
学术爱丽丝是你的 **学术桌面助手**：陪你盯进度、喘口气，科研工作台上的论文与项目也跟着稳住节奏。

**中版（一段）**  
学术爱丽丝运行在 Windows 桌面一侧，以桌宠形式常驻；科研工作台在浏览器里陪你整理论文、项目和日常任务，数据以本地为主，界面与操作保持简单。
...
学术爱丽丝 **不依赖** 用户安装《鸣潮》。**粉青像素与边框层次** 保留。
...
- 新增面向用户的字符串时，优先 **`from app_brand import APP_DISPLAY_NAME, APP_TAGLINE`**，避免硬编码「学术爱丽丝」四处漂移。
- 若英文 README 仍以仓库原名为标题，可在首段用中文或英文加一行 **Fork 显示名：Academic Alice / 学术爱丽丝**，与根目录 `README.md` 当前做法一致。
