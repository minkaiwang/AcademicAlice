...
- **deskpet.spec** + **打包Windows.bat** + **read_py_exe.ps1** — PyInstaller 生成 `dist/AcademicAlice/`。spec 对 PyQt5 使用 `collect_all`，打包机须已 `pip install PyQt5`；**建议用 Python 3.11–3.12 打正式包**（3.14 等过新版本易出现 Qt 收集不全）。发给他人时需 **整个 `AcademicAlice` 文件夹** zip 解压后运行其中 exe，对方 **无需安装 Python**。
