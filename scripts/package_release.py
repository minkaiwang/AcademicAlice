...
def _git_file_paths(*, include_untracked: bool = False) -> list[Path]:
    ...
    try:
        ...
        raise RuntimeError("源码包必须在可用的 Git 工作树中生成") from exc
    return [
...
def _iter_files(*, include_untracked: bool = False) -> Iterator[FileEntry]:
    for rel in _git_file_paths(include_untracked=include_untracked):
        path = ROOT / rel
        try:
            if not path.exists():
                continue
...
def parse_args(argv: List[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Package AemeathDeskPet release bundle.")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    zip_path = args.output / f"AemeathDeskPet-{args.version}.zip"
    manifest_path = args.output / f"AemeathDeskPet-{args.version}-manifest.json"
    ...
    print(f"[package] wrote {checksum_path.relative_to(ROOT)} ({digest})")
    return 0
